            ...
            for (const episode of season.episodes) {
              ...
              if (episode.air_date >= sevenDaysAgoStr && episode.air_date <= today) {
                ...
                const existingFiles = await prisma.file.findMany({
                  where: {
                    mediaItemId: show.id,
                    parsedSeason: seasonNum,
                    parsedEpisode: episode.episode_number,
                  },
                });
...
export async function autoDownloadEpisode(episode: NewEpisode): Promise<boolean> {
  ...
  try {
    ...
    if (!media.tmdbId) {
      console.error('  ❌ Media has no TMDB ID, cannot generate search queries');
      return false;
    }
    ...
    const searchData = {
      ...
      releaseYear: media.year ?? undefined,
    ...
    // Get templates
    const showOverride = (settings.searchTemplates?.overrides as any)?.[media.tmdbId || 0];
    ...
    const results = await performUnifiedSearch(queries[0], 'tv', media.year ?? undefined, settings);
