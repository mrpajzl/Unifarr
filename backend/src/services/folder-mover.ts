...
export async function moveMediaFolder(
  ...
  try {
    ...
    if (!media) {
      return { success: false, oldPath: '', newPath, filesMoved: 0, error: 'Media item not found' };
    }

    const oldPath = media.libraryPath;
    if (!oldPath) {
      return { success: false, oldPath: '', newPath, filesMoved: 0, error: 'No library path set for this media item' };
    }
    ...
    if (autoMove) {
      try {
        ...
        try {
          ...
          console.warn(`Failed to remove old directory: ${oldPath}`);
        }
      } catch (error) {
        return {
          success: false,
    ...
    console.log(`✅ Updated ${relatedFiles.length} file records`);
 ...
 */
export async function validatePath(filePath: string): Promise<{ valid: boolean; error?: string }> {
