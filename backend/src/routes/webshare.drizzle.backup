...
app.post('/download', async (c) => {
  try {
    ...
    if (!finalTargetPath && mediaId) {
      // Try to get media item to determine path
      const { mediaItems } = await import('../db/schema');
      const { db } = await import('../db');
      const media = await db.query.mediaItems.findFirst({
        where: (mediaItems, { eq }) => eq(mediaItems.id, mediaId),
      });
      ...
      if (media) {
        ...
        // Create folder name for the media (e.g., "Avatar (2009)")
        const folderName = media.year 
          ? `${media.title} (${media.year})`
          : media.title;
        
        // Combine to create full target path
