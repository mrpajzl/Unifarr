...
router.get('/:mediaId/files', async (c) => {
  ...
  const matched = parsedFiles.filter(f => f.isMatched);
  const unmatched = parsedFiles.filter(f => !f.isMatched);
...
router.post('/:mediaId/apply-pattern', async (c) => {
  ...
  // Get all unmatched files (no season OR no episode)
  const unmatchedFiles = await prisma.file.findMany({
    where: {
      mediaItemId: mediaId,
      OR: [
        { parsedSeason: null },
