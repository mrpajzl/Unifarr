...
export default function SonarrSettingsPage() {
  ...
  const loadAllSettings = async (cfg: AppConfig) => {
    ...
    try {
      const [configRes, qualityRes, rootRes, languageRes, downloadRes, indexerRes, notificationRes, tagRes] = await Promise.all([
        fetch('/api/proxy', {
          method: 'POST',
      ...
      if (languageRes.ok) setLanguageProfiles(await languageRes.json());
  ...
  const tabs = [
    ...
    { id: 'language', label: 'Language Profiles' },
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
            ...
            <div className="p-6">
              ...
              {activeTab === 'language' && (
                <div className="space-y-4">
                  <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Language Profiles</h2>
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                      <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                        ...
                        {languageProfiles.map((profile) => (
                          <tr key={profile.id}>
                            ...
                            <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                              {profile.languages?.map((l: any) => l.name).join(', ') || 'N/A'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tags.map((tag) => (
                      <span
                        key={tag.id}
                        className="px-3 py-1 bg-sonarr text-white rounded-full text-sm"
                      >
                        {tag.label}
                      </span>
                    ))}
